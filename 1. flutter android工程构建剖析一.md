...
使用VS Code创建一个最简单的`hello_world `工程：

![](images/1_1_create_project.png)

# 分析

我们看生成的`hello_world `工程由很多个文件和目录，我们**主要先看一下android目录**：
...
### 和普通android工程一样添加了`android gradle 插件`和`kotlin gradle 插件`依赖，**没有添加额外的插件**：
...
### 把编译的临时目录都改到`flutter`工程根目录的`build`目录下：
...
比如在正常情况下（没有以上代码），都会在当前工程下创建一个`build`目录作为编译的临时产物目录：
...
但是通过以上代码，把所有工程的中间目录都给统一到了`flutter`工程根目录的`build`目录下，最后目录结构如下：
	...
	│   ├── local.properties
	│   └── settings.gradle
	├── build // -> 根工程的build目录，没有变化
	│   ├── app // -> app工程的build目录被挪动到了这里
	│   │   ├── generated
	│   │   ├── intermediates
	│   │   ├── kotlin
	│   │   ├── outputs
...
# 看完根工程的`build.gradle，然后我们看一下`app`工程下面的`build.gradle`
...
## 读取根工程的`local.properties`文件，在生成flutter工程的时候，会生成一个`local.properties`文件，位置如下：
	...
	│   ├── gradlew.bat
	│   ├── hello_world_android.iml
	│   ├── local.properties // <-生成的local.properties文件，在根工程的目录下
	│   └── settings.gradle
...
其中`flutter.versionName`和`flutter.versionCode`定义App的版本号：
...
整体`flutter`打包脚本的核心就是这个文件，也是普通Android App工程和`flutter`工程打包脚本的唯一关键差异性。
...
后面可以通过`project.flutter.source`这么来获取上面赋值的`source`值：
...
从以上代码我们看出，读取`flutter`目录下的`bin/internal/engine.version`文件里面包含了版本号：
...
`flutter`默认打的是`ARM32`和`ARM64`的，如果是`debug`包那么还会打`x86`和x64的，因为要给模拟器运行。
...
1.  `-dTargetFile=lib/main.dart` // flutter的入口dart文件
1.  `-dTargetPlatform=android`
1. `-dBuildMode=debug`
...
要运行`flutter`工程，一定要有一个`flutter`运行环境一起被打倒最终包里面去，我们看上面有一个`libflutter.so`：
